...
estimateTheta <- function(transcripts, bamfiles, fitpar, genome,
  ...
  res <- lapply(seq_along(bamfiles), function(i) {
    ...
    lapply(model.names, function(modeltype) {
...
estimateThetaSingleIsoform <- function(transcripts, bamfiles, fitpar, genome,
                                       models, readlength,
                                       subset=FALSE, zerotopos, niter, 
                                       lib.sizes=NULL, optim=FALSE, minsize, maxsize) {
  stopifnot(is(transcripts, "GRangesList"))
  ...
  # this is a list of fragment types for each transcript
  st <- system.time({ 
      out <- buildFragtypesFromExons(transcripts[[1]], genome, readlength, minsize, maxsize)
    })
  out$tx <- names(transcripts)
  fraglist <- list(out)
  ...
  res <- lapply(seq_along(bamfiles), function(i) {
    ...
    # from here below, specific code for when we have one isoform
